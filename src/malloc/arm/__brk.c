...
// User code can use linker script to define the heap region.
extern uintptr_t __libc_heap_start __attribute__((weak, visibility("default")));
extern uintptr_t __libc_heap_end __attribute__((weak, visibility("default")));
#endif
...
uintptr_t __brk(uintptr_t newbrk)
{
  ...
  if (newbrk == 0) {
    ...
    if (&__libc_heap_start && &__libc_heap_end &&
        !__heap_base && !__heap_limit) {
      __heap_base = (uintptr_t)(&__libc_heap_start);
      __heap_limit = (uintptr_t)(&__libc_heap_end);
    }
    return __heap_base;
  }
